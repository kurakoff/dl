    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {filters.map((f, i) => (
        <div key={f.id} className="flex items-center gap-1">
          {i > 0 && (
            <span className="text-xs font-semibold px-1.5 py-0.5 rounded-lg bg-gray-100 text-gray-500">
              {f.connector}
            </span>
          )}
          <div className="flex items-center gap-1 px-2.5 py-1.5 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-700 font-medium">
      ...
      <div className="relative" ref={ref}>
        ...
        {open && (
          <div
            className="absolute top-full left-0 mt-1.5 z-50 bg-white border border-gray-200 rounded-2xl shadow-xl p-4"
            style={{ width: '300px' }}
          >
                ...
                <div key={row.id}>
                  ...
                  {i > 0 && (
                    <div className="flex items-center gap-2 my-1.5">
                      ...
                      >
                        <span className={`px-2 py-0.5 transition ${row.connector === 'AND' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-50'}`}>
                          AND
                        </span>
                        <span className={`px-2 py-0.5 transition ${row.connector === 'OR' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:bg-gray-50'}`}>
                          OR
                        </span>
                      </button>
                      <div className="flex-1 h-px bg-gray-100" />
                  ...
                  <div className="flex items-center gap-1.5">
                    <select
                      ...
                      className="flex-1 min-w-0 text-sm border border-gray-200 rounded-lg px-2 py-1.5 outline-none focus:border-blue-400 bg-white text-gray-700"
                    >
                      {METRICS.map(m => (
                    ...
                    <select
                      ...
                      className="w-14 text-sm border border-gray-200 rounded-lg px-1.5 py-1.5 outline-none focus:border-blue-400 bg-white text-gray-700 text-center"
                    >
                      {OPS.map(o => (
                    ...
                    <input
                      ...
                      placeholder="0"
                      className="w-14 text-sm border border-gray-200 rounded-lg px-2 py-1.5 outline-none focus:border-blue-400"
                    />
            ...
            <button
              onClick={addRow}
              className="mt-3 flex items-center gap-1 text-xs text-blue-500 hover:text-blue-700 transition font-medium"
            >
